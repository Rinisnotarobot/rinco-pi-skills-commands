...
- `bash` with `rg` for finding test smells (.skip, .only, missing assertions)
- Write for creating tests/README.md
- Pi `todo` tool for tracking cleanup steps
...
- **`bash` with `rg`**: Find test smells and quality issues (direct)
- **`bash` with `find`/`rg --files`**: Find test files by pattern (direct)
- **Write**: Create test documentation (direct)
- **Edit**: Update .gitignore (direct)
- **Pi `todo` tool**: Track multi-step cleanup process (direct)
...
Use Pi `todo` tool to track progress:
