...
- `bash` with `find`/`rg --files` for finding documentation files
...
- **`bash` with `find`/`rg --files`**: Find documentation by pattern (direct)
- **Edit**: Update .gitignore (direct)
- **Pi `todo` tool**: Track cleanup steps (direct)
