...
- `bash` with `rg` for import/reference searching
...
- **`bash` with `rg`**: Search for imports and references (direct)
- **`bash` with `find`/`rg --files`**: Find files by pattern for cleanup (direct)
