...
When the scope adds or changes a remote dependency, durable queue or worker, multi-step side effect, deadline, cancellation, retry, idempotency, concurrency limit, backpressure, degradation, or recovery path, require a same-state `resilience` assessment. Invoke `resilience` when it is available; otherwise stop with `BLOCKED`, name `resilience` as the missing Skill, and provide the restart instruction: install it into the Pi skill discovery path and restart the session, or temporarily launch with `pi --no-skills --skill skills/workflows/verification --skill skills/patterns/resilience --skill skills/tools/terminal-ops`. A required resilience assessment that is missing, stale, or blocked prevents `READY`.
