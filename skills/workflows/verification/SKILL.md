...
Collect upstream artifacts and evidence when present: the source specification and originating `REQ`/`INV`/`AC` identifiers, the plan's verification contract, systematic-debugging cause and reproduction handoff, TDD RED/GREEN handoff, backend invariant, resilience policy and failure-evidence requirements, review scope, and explicit user acceptance. Follow [Evidence Handoffs](references/evidence-handoffs.md) to accept only same-state evidence and reject stale or incomplete results.

When the scope adds or changes a remote dependency, durable queue or worker, multi-step side effect, deadline, cancellation, retry, idempotency, concurrency limit, backpressure, degradation, or recovery path, require a same-state `resilience` assessment. Invoke `resilience` when it is available; otherwise stop with `BLOCKED`, name `resilience` as the missing Skill, and give its installation destination and an exact `pi --no-skills --skill <path> ...` restart command derived from this installation. Preserve `verification`, `resilience`, `terminal-ops`, and any upstream owner that must resume; mark missing paths as installation prerequisites. Use verified installed paths, not source-checkout examples, and include the next explicit invocation. A required resilience assessment that is missing, stale, or blocked prevents `READY`.
