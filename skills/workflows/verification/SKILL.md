...
Reuse upstream evidence when present — the source specification and originating `REQ`/`INV`/`AC` identifiers, a plan's verification contract, a debugging cause and reproduction record, TDD RED/GREEN records, a resilience policy, a review scope, or explicit user acceptance. Apply the acceptance test in [Evidence Handoffs](references/evidence-handoffs.md): reuse a result only when it is same-state and proves the current gate's claim; otherwise rerun it or mark the gate blocked. Missing upstream evidence is normal — verify the claim from repository evidence directly, and never presume an outcome from an earlier run.

When the change touches remote dependencies, durable queues or workers, multi-step side effects, deadlines, cancellation, retry, idempotency, concurrency limits, backpressure, degradation, or recovery paths, required gates must cover that failure and recovery surface: the repository's own durability, consistency, and retry checks, or a resilience review when one exists. Do not mark that surface `N/A` merely because no review was run.
...
Use configured thresholds and scripts. A test pass does not prove a build, a lint pass does not prove types, and one package does not prove an affected workspace. This phase runs only configured gates and direct change-integrity checks; judgment about design, maintainability, correctness-by-inspection, product acceptance, and release authorization belongs to the review and approval steps that follow, not to the gate verdict.
