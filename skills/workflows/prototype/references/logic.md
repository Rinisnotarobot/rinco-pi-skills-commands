...
If the question is "what should this look like," this is the wrong branch. Use [UI Prototype](ui.md).

## Process

### 1. State the question

Before writing code, write down what state model and what question is being prototyped, plus the stopping condition from the main skill. One paragraph, at the top of the demo (in a visible intro, not just a comment). A logic prototype that answers the wrong question is pure waste, so make the question explicit so it can be checked later, whether the user is watching now or returning to it later.
...
Send them the file, or open it for them. They'll click through the walkthroughs and free-play whenever they get to it; the interesting moments are when they say "wait, that shouldn't be possible" or "huh, I assumed X would be different"; those are the bugs in the _idea_, which is the whole point. If they want new actions or a new scenario, add them — refining the exploration of the same question is fine; a genuinely new question is a new prototype, not an extension of this one.
