...
Ask only about decisions that cannot be discovered and would materially change behavior, safety, compatibility, persistent data, cost, or public interfaces. When unresolved product or policy truth affects any of those outcomes or acceptance meaning, confirm that `spec` is available, then stop and ask the user to invoke `/skill:spec`; `spec` is user-invoked and planning must not synthesize it silently. When it is unavailable, return `BLOCKED`, name its installation destination, and give an exact `pi --no-skills --skill <path> ...` restart command preserving `plan` and adding `spec` from this installation, followed by the explicit `/skill:spec` request. Mark the missing destination as an installation prerequisite instead of using an unresolved `/path/to` placeholder. Only technical assumptions that do not alter desired behavior or safety may remain open in a persisted plan.
