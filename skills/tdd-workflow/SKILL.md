...
1. **Detect the package manager from project evidence.** Inspect, in order: `package.json#packageManager`, then exactly one authoritative lockfile (`pnpm-lock.yaml`, `bun.lock`/`bun.lockb`, `yarn.lock`, or `package-lock.json`). If evidence conflicts, ask which manager is authoritative. Do not invoke a repository-local detector or install another manager automatically.
...
For Python projects, inspect `pyproject.toml` and existing test configuration. If `uv` is missing, install it automatically with Astral's official standalone installer, refresh `PATH`, and verify `uv --version`; installing `uv` itself does not require approval. Use `uv run pytest <scope>` and `uv run pytest --cov <scope>` only when pytest and coverage are configured; use `uv run` for configured lint/type tools. Never invoke bare Python tooling or install test dependencies without approval.
...
When the project uses Bun's built-in runner (see [Step 0](#step-0-detect-the-test-runner)), import from `bun:test` and run with `bun test` — not `bun run test`. The API is Jest-like, so `describe` / `it` / `expect` and most matchers carry over. Consult the project's Bun configuration and current Bun documentation for runtime-specific details.
...
```bash
# Run manually or wire into the project's existing hook system
<test> && <lint>
```

Do not create or modify hooks without explicit user approval.
