...
description: Analyze an unfamiliar codebase and generate a structured onboarding guide with architecture map, key entry points, conventions, and starter AGENTS.md instructions for Pi. Use when joining a new project or setting up Pi in a repository for the first time.
metadata:
  origin: ECC
---
...
Systematically analyze an unfamiliar codebase and produce a structured onboarding guide. Designed for developers joining a new project or setting up Pi in an existing repository for the first time.
...
Generate or update a project-specific `AGENTS.md` based on detected conventions. If `AGENTS.md` already exists, read it first and enhance it—preserve existing project-specific instructions and clearly call out what changed. If only another harness's legacy instruction file exists, treat it as migration input, preserve relevant instructions in `AGENTS.md`, and leave the legacy file untouched unless the user asks to remove it.
...
1. **Don't read everything** — use Pi's `codegraph_codegraph_explore` first when an index is available; otherwise use `bash` with `find`/`rg`. Read files selectively for ambiguous signals.
2. **Verify, don't guess** — if a framework is detected from config but the actual code uses something different, trust the code.
3. **Respect existing AGENTS.md** — enhance it rather than replacing it, and call out what's new versus existing.
...
- Generating an `AGENTS.md` longer than 100 lines—keep it focused
...
**Output**: Onboarding Guide printed directly to the conversation, plus an `AGENTS.md` written to the project root

### Example 2: Generate AGENTS.md for an existing project
...
**User**: "Update AGENTS.md with current project conventions"
**Action**: Read existing `AGENTS.md`, run Phases 1-3, and merge new findings
**Output**: Updated `AGENTS.md` with additions clearly identified
